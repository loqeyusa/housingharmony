...
export interface SteeleClientData {
  caseNumber?: string;
  clientName: string;
  properties: string;
...
export function cleanCurrency(amount?: string): string {
  ...
  // Handle "case inactive" or other special status values
  if (amount.toLowerCase().includes("case inactive") || amount.toLowerCase().includes("inactive")) {
    return "0.00";
  }
  
  // Remove $ signs, commas, extra spaces, and "$ -" patterns
...
export async function findOrCreateSteeleProperty(
  ...
  if (existingBuilding.length > 0) {
    ...
    // Create new building for Steele County
    const [newBuilding] = await db
      ...
      .values({
        ...
        landlordPhone: "(507) 444-0000", // Default Steele County phone
        landlordEmail: "info@co.steele.mn.us", // Default Steele County email
        totalUnits: 1,
...
export async function upsertSteeleClient(clientData: SteeleClientData, companyId: number = 1): Promise<void> {
  ...
  if (!firstName || !lastName) {
    console.log(`Skipping Steele client with invalid name: ${clientData.clientName}`);
    return;
  }

  // Find or create property
  const propertyId = await findOrCreateSteeleProperty(
    clientData.properties,
    clientData.rentalOfficeAddress,
    companyId
  );

  // Determine status based on rent amount or notes
  const isInactive = clientData.rentAmount?.toLowerCase().includes("case inactive") || 
                     clientData.notes?.toLowerCase().includes("case inactive") ||
                     clientData.rentAmount?.toLowerCase().includes("inactive");
  ...
  const clientPayload = {
    ...
    email: `${firstName.toLowerCase()}.${lastName.toLowerCase()}@co.steele.mn.us`,
    phone: "(507) 444-0000", // Default phone since not provided
    ...
    monthlyIncome: "0.00",
    county: "Steele", // All clients are Steele County
    propertyId,
  ...
  try {
    ...
    if (existingClient) {
      ...
      console.log(`Updated Steele client: ${firstName} ${lastName} (ID: ${existingClient.id})`);
    } else {
      // Insert new client
      ...
      console.log(`Created new Steele client: ${firstName} ${lastName} (ID: ${newClient.id})`);
    }
  } catch (error) {
    console.error(`Error processing Steele client ${firstName} ${lastName}:`, error);
  }
}
...
export function parseSteeleCSVLine(line: string): SteeleClientData | null {
  // Skip header row
...
export async function importSteeleCSVFile(filePath: string, companyId: number = 1): Promise<{
  processed: number;
  created: number;
  updated: number;
  skipped: number;
  ...
  console.log(`Starting Steele County CSV import of ${lines.length} lines...`);

  for (const line of lines) {
    if (line.trim() === '') continue;
    
    const clientData = parseSteeleCSVLine(line);
    if (!clientData) {
      skipped++;
      continue;
    }
    ...
    try {
      await upsertSteeleClient(clientData, companyId);
      processed++;
    } catch (error) {
      console.error(`Error processing Steele line: ${line.substring(0, 100)}`, error);
      skipped++;
    }
  }

  console.log(`Steele County import completed: ${processed} processed, ${created} created, ${updated} updated, ${skipped} skipped`);
