...
export interface HennepinClientData {
  clientName: string;
  properties: string;
...
export async function findOrCreateHennepinProperty(
  ...
  if (existingBuilding.length > 0) {
    ...
    // Create new building for Hennepin County
    const [newBuilding] = await db
      ...
      .values({
        ...
        landlordPhone: "(612) 000-0000", // Default Hennepin County phone
        landlordEmail: "info@hennepin.us", // Default Hennepin County email
        totalUnits: 1,
...
export async function upsertHennepinClient(clientData: HennepinClientData, companyId: number = 1): Promise<void> {
  ...
  if (!firstName || !lastName) {
    console.log(`Skipping Hennepin client with invalid name: ${clientData.clientName}`);
    return;
  }

  // Find or create property
  const propertyId = await findOrCreateHennepinProperty(
  ...
  const clientPayload = {
    ...
    email: `${firstName.toLowerCase()}.${lastName.toLowerCase()}@hennepin.us`,
    phone: "(612) 000-0000", // Default phone since not provided
    ...
    monthlyIncome: "0.00",
    county: "Hennepin", // All clients are Hennepin County
    propertyId,
  ...
  try {
    ...
    if (existingClient) {
      ...
      console.log(`Updated Hennepin client: ${firstName} ${lastName} (ID: ${existingClient.id})`);
    } else {
      // Insert new client
      ...
      console.log(`Created new Hennepin client: ${firstName} ${lastName} (ID: ${newClient.id})`);
    }
  } catch (error) {
    console.error(`Error processing Hennepin client ${firstName} ${lastName}:`, error);
  }
}
...
export function parseHennepinCSVLine(line: string): HennepinClientData | null {
  // Skip header row
...
export async function importHennepinCSVFile(filePath: string, companyId: number = 1): Promise<{
  processed: number;
  created: number;
  updated: number;
  skipped: number;
  ...
  console.log(`Starting Hennepin County CSV import of ${lines.length} lines...`);

  for (const line of lines) {
    if (line.trim() === '') continue;
    
    const clientData = parseHennepinCSVLine(line);
    if (!clientData) {
      skipped++;
      continue;
    }
    ...
    try {
      await upsertHennepinClient(clientData, companyId);
      processed++;
    } catch (error) {
      console.error(`Error processing Hennepin line: ${line.substring(0, 100)}`, error);
      skipped++;
    }
  }

  console.log(`Hennepin County import completed: ${processed} processed, ${created} created, ${updated} updated, ${skipped} skipped`);
